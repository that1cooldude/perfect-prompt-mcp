...
interface PromptStructure {
  type: 'plain' | 'xml' | 'json' | 'markdown' | 'role-based';
  preserveFormat: boolean;
}

export class OpenRouterClient {
  private static readonly API_URL = 'https://openrouter.ai/api/v1/chat/completions';
  private static readonly DEFAULT_MODEL = 'google/gemini-2.5-flash-preview-05-20';
  private model: string;
  private static readonly SYSTEM_PROMPT = `You are a prompt enhancement assistant. Your ONLY job is to rewrite prompts to be clearer, more specific, and more effective.
...
Output: "Explain quantum computing in accessible terms, covering the fundamental principles of superposition and entanglement, how qubits differ from classical bits, current practical applications, major technical challenges, and potential future breakthroughs that could transform computing"`;
  ...
  private static readonly STRUCTURED_SYSTEM_PROMPT = `You are a prompt enhancement assistant. Your ONLY job is to rewrite prompts while preserving their exact structure and format.
...
8. Enhance ONLY the content within the structure, never modify the structure itself`;
  ...
  async enhancePrompt(prompt: string, conversationContext?: string): Promise<string> {
    ...
    try {
      ...
      const data = await response.json() as any as OpenRouterResponse;
  ...
  private detectPromptStructure(prompt: string): PromptStructure {
  ...
  private processThinkingModelResponse(content: string): string {
  ...
  private buildContextualPrompt(conversationContext: string): string {
    return `
...
Based on this conversation history, enhance the user's prompt to be more specific, contextually relevant, and aligned with their ongoing work.`;
  ...
  private async makeRequest(prompt: string, structure: PromptStructure, conversationContext?: string): Promise<Response> {
  ...
  private calculateMaxTokens(prompt: string): number {
